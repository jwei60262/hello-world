...
异步有两种解释：既可以可以是问题、环境、挑战、客观现实，也可以是异步线程这种解决方法。

## JS 异步模型

Nodejs 设计是事件驱动的单线程非阻塞模型。将临时产生的异步任务放到队列尾部，当前任务处理完成，依次处理。

## 网络编程中三个核心 I/O 模型

### 阻塞 I/O (BIO)

BIO (Blocking I/O) 是最传统、最简单的 I/O 模型。

核心机制：进程/线程阻塞

当应用程序发起一个 I/O 操作（如 `read()` 或 `write()`）时，如果数据尚未准备好，应用程序的线程会被挂起（阻塞），直到数据准备就绪，并被内核复制到用户空间后，线程才会被唤醒并返回。

- 同步性： 它是同步阻塞的。
- 架构特点： 通常采用 “一个连接，一个线程” 的模型。

**优点 (Pros)**

- 编程简单： 代码逻辑直观，易于理解和实现。

- CPU 资源利用高： 线程在等待时被挂起，不占用 CPU 时间。

**缺点 (Cons)**

- 并发性低： 每个连接都需要一个专门的线程，资源消耗大。
- 性能瓶颈： 在高并发场景下，会创建大量线程，导致系统开销和线程上下文切换成本极高。

### 非阻塞 I/O (NIO)

NIO (Non-blocking I/O) 旨在解决 BIO 的线程阻塞问题。

核心机制：忙轮询 (Busy Polling)

当应用程序发起一个 I/O 操作时，如果数据尚未准备好，系统调用会立即返回一个错误或状态码，而不会阻塞线程。应用程序必须不断地（或非常频繁地）向内核发送请求，询问“数据好了吗？”。这种反复询问数据的过程称为 轮询（Polling）。

- 同步性： 它是同步非阻塞的。
- 架构特点： 线程不会被挂起，但需要不断地进行系统调用来检查状态。

**优点 (Pros)**

- 线程资源少： 单个线程可以处理多个连接的 I/O 操作，并发性提升。

**缺点 (Cons)**

- CPU 消耗高： 线程在等待数据期间持续进行轮询，导致大量的 CPU 时间浪费在无效的系统调用上（即“忙等待”）。
- 效率低下： 大多数轮询都是徒劳的，效率远低于后面的多路复用。无效轮询带来线程上下文切换成本。

### I/O 多路复用 (I/O Multiplexing)

I/O 多路复用是高性能网络编程中最常用的模型，它结合了非阻塞和事件驱动的思想，有效解决了 BIO 和 NIO 的弊端。单次 select() 轮询可以处理多个连接，如果没有事件产生，就一直阻塞，不需要频繁轮询，占用CPU。

核心机制：事件通知 (Event Notification)

应用程序不是自己去轮询所有连接，而是将所有关注的 I/O 流（文件描述符/Channel）注册到一个选择器 (Selector) 上。IO 线程阻塞在单一的 select、poll 或 epoll 等多路复用函数上。

- 同步性： 它是同步阻塞的（阻塞在 select 调用上）。
- 架构特点： 使用 单个线程 或少量线程，管理数千个连接。

**优点 (Pros)**

- 高并发性： 单个线程能有效处理大量并发连接，并发性能极高。
- 资源消耗低： 仅在有 I/O 事件发生时才唤醒线程，避免了 BIO 的大量线程开销和 NIO 的无效 CPU 轮询。

**缺点 (Cons)**

- 编程复杂： 涉及 Selector、Channel、Buffer 等概念，代码逻辑比 BIO 复杂得多。
- 仍属同步模型： 在数据准备就绪后，内核将数据复制到用户空间的过程仍然是阻塞的。

**补充：**

I/O 多路复用是事件驱动的非阻塞式机制，将临时任务标记为事件，不阻塞当前任务，当前任务处理完成之后，select/epoll 遍历到新的事件队列后处理。比如 Netty 正在处理读任务，临时产生了一个写操作，这个写操作不会立即执行、阻塞当前任务，而是调用内核方法、修改文件描述符，标记为写事件，在下一轮遍历到新事件队列时处理。
...
| 机制            | 代表系统调用                                 | 适用场景                                                     |
| --------------- | -------------------------------------------- | ------------------------------------------------------------ |
| **select/poll** | Unix/Linux 系统早期，适用于中低并发。        | 连接数有限，性能要求不高。                                   |
| **epoll**       | Linux 独有，基于事件，性能远超 select/poll。 | **当前高性能服务器首选**，适用于高并发场景（如 Nginx、Redis）。 |
